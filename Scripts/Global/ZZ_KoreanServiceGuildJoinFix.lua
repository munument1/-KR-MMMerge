-- MMMerge Korean patch: restore the six MM6 service/mercenary guild topics
-- that are present in Global.evt but do not respond in the Merge runtime.

local PendingServiceGuildTopic = 0

function events.EnterNPC()
	PendingServiceGuildTopic = 0
end

local function JoinServiceGuild(TopicId, Cost, InfoText, AutonoteBit)
	if PendingServiceGuildTopic ~= TopicId then
		PendingServiceGuildTopic = TopicId
		Message(Game.NPCText[InfoText])
		return
	end

	if Party.AutonotesBits[AutonoteBit] then
		Message(Game.NPCText[124]) -- Already a member of this guild.
	elseif Party.Gold >= Cost then
		evt.Subtract{"Gold", Cost}
		evt.Add{"AutonotesBits", AutonoteBit}
		Message(Game.NPCText[InfoText])
	else
		Message(Game.GlobalTxt[155]) -- Not enough gold.
	end

	FirstClick = false
	PendingServiceGuildTopic = 0
end

local function SetServiceGuildTopic(TopicId, Cost, InfoText, AutonoteBit)
	Game.GlobalEvtLines:RemoveEvent(TopicId)
	evt.Global[TopicId]:clear()
	evt.Global[TopicId] = function()
		JoinServiceGuild(TopicId, Cost, InfoText, AutonoteBit)
	end
end

SetServiceGuildTopic(1696, 25, 1832, 629) -- Buccaneers' Lair
SetServiceGuildTopic(1697, 50, 1833, 630) -- Protection Services
SetServiceGuildTopic(1698, 50, 1834, 631) -- Smugglers' Guild
SetServiceGuildTopic(1699, 25, 1835, 632) -- Blade's End
SetServiceGuildTopic(1700, 50, 1836, 633) -- Duelists' Edge
SetServiceGuildTopic(1701, 50, 1837, 634) -- Berserkers' Fury
