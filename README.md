...
## v1.0.6 기존 세이브 NPC 이름 수정

- 번역 전에 생성된 세이브에 `Margaret the Docent` 같은 영문 NPC 이름이 남는 문제를 수정했습니다.
- 영문 NPC 이름과 한국어 이름의 대응을 로딩할 때 보존하고, 맵 로드 후 NPC에 복사된 이름도 한국어로 변환합니다.
- 세이브에 영구 저장된 무작위 NPC 이름도 함께 변환하므로, 다시 저장하고 불러와도 영문 이름으로 돌아가지 않습니다.
- 고정 NPC `Margaret the Docent`는 `도슨트 마거릿`으로 표시됩니다.
...
v1.0.6은 다음 항목을 확인했습니다.

- 대화 2,713개 ID와 서식 토큰 보존
- 무작위 NPC 이름 남성 539개·여성 310개 런타임 적용
- 고정 NPC 이름 1,148개와 무작위 NPC 이름 840개의 영문→한국어 대응 충돌 없음
- 기존 세이브의 복사·영구 저장 NPC 이름 변환 및 실제 게임 표시 확인
- 주문명 132개 런타임 적용
